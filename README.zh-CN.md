...
`creditlint` 是一个用于强制执行 Git 署名和贡献元数据策略的 Rust 原生命令行工具。

它帮助允许 AI 辅助开发的团队阻止未经授权的署名或贡献标记，例如 `Co-authored-by`、`Made with` 或生成工具尾注，这些可能出现在提交、拉取请求和合并消息中。
...
这些元数据对于溯源可能有用，但不应悄然成为署名或贡献归属。`creditlint` 保持这些概念的分离：

- 署名和贡献标记由策略控制。
- 溯源标记可以在不暗示署名的情况下被允许。
- 强制执行通过本地钩子、CI、仓库规则或合并机器人进行。

## 当前状态

MVP CLI 已实现，仓库正在准备交付。
...
- 可选的 npm 包装器，适用于已通过 npm、pnpm 或 npx 安装开发工具的团队
...
如果没有 `.creditlint.yml`，`creditlint` 使用内置默认策略，阻止常见的 AI/工具署名和贡献标记，同时允许显式的溯源尾注。
...
`creditlint` 设计为在多个层级运行，因为没有单个钩子或 CI 任务能看到所有 Git 元数据表面。
...
边界很重要：`creditlint check --range` 验证提议的提交，但本身不验证由托管平台 UI 编辑或合成的最终压缩合并消息。

有关在其他仓库中的强大部署指导，请使用仓库本地技能：
